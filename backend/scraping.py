...
def get_one_adict(link: str):
    ...
    e_id = soup.find(name= 'div', class_='addicon__name').text
    name = soup.find(name= 'span', class_= 'field--title').text
    danger_level = soup.find(name= 'div', class_='addprop--danger'
                            ).find(name= 'a', class_= 'addprop-item'
                            ).text
    e_type =  [item.string for item in soup.find(name= 'div', class_='addprop--category'
                            ).find_all(name= 'a', class_= 'addprop-item')]
    origins =  [origin.string for origin in (soup.find(name= 'div', class_='addheader__right-top').find_all('a'))]
    description = soup.find(name= "div", class_='field--additive-info'
                            ).find('p'
                            ).text
